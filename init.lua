...
--[[]]
if comspec and comspec:match 'powershell.exe' then
  avante_build_command = 'powershell -ExecutionPolicy Bypass -File Build.ps1 -BuildFromSource false'
elseif nvim_shell and nvim_shell:match 'pwsh' then
  avante_build_command = 'powershell -ExecutionPolicy Bypass -File Build.ps1 -BuildFromSource false'
end
--[[]]
...
require('lazy').setup(
  {
    ...
    --[[]]
    {
      ...
      -- Default settings from avante github repo
      -- opts = {
      --      provider = 'openai',
      --      openai = {
      ...
      -- we use advanced mode like mentioned on website,
      -- https://nithinbekal.com/posts/avante-nvim/
      config = function()
        local opts = { provider = 'copilot' }

        local use_openai = false
        local open_api_url = os.getenv 'OPEN_API_CHAT_COMPLETITIONS'
        if use_openai and open_api_url then
          opts = {
            provider = 'openai',
            openai = {
              endpoint = 'https://api.openai.com/v1',
    ...
    -- not just copy pasted the init.lua. If you want these files, they are in the repository,
